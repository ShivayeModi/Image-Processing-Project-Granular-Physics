...
import os
"""
go to jth frame>go to ith ring > print velocity of ith ring at jth frame>complete iteration for all rings in jth frame>find maxima of the 
the ring velocity>this will be used as v_max
"""
#os.chdir("F:\\summer project\\image processing\\cross correlation")
#os.mkdir("piv_set_diff_10_framescale")
for j in range(0,15): # j as frame index
 ...
 for i in range(1,175): # i as ring index
    ...
    arrow_length = int((40 / v_max) * (vel_x[j]))
    xnew = x0[j] + arrow_length * np.cos(np.pi / 2 + phi[j])  # dont go in normal cartesian frame,think about cv window frame
    ynew = y0[j] - arrow_length * np.sin(np.pi / 2 + phi[j])
    if vel_x[j] == 0:
        cv2.circle(img, (int(x0[j]), int(y0[j])), 3, (255, 0, 255), -1)
    else:
        cv2.arrowedLine(img, (int(x0[j]), int(y0[j])), (int(xnew), int(ynew)), (0, 255, 255),2,tipLength=2)
 print(j)
 cv2.destroyAllWindows()
 cv2.waitKey(1)
 cv2.imwrite("F:\\summer project\\image processing\\cross correlation\\piv_set_diff_10"+"\\"+"frame_"+str(j)+"diff_10"+".jpg",img)
